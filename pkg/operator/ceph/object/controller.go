...
func (c *ObjectStoreController) onAdd(obj interface{}) {
	objectstore, migrationNeeded, err := c.getObjectStoreObject(obj)
	if err != nil {
...
func (c *ObjectStoreController) onUpdate(oldObj, newObj interface{}) {
	...
	oldStore, _, err := c.getObjectStoreObject(oldObj)
	if err != nil {
		logger.Errorf("failed to get old objectstore object: %+v", err)
		return
	}
	newStore, migrationNeeded, err := c.getObjectStoreObject(newObj)
	if err != nil {
...
func (c *ObjectStoreController) getObjectStoreObject(obj interface{}) (objectstore *cephv1alpha1.ObjectStore, migrationNeeded bool, err error) {
	var ok bool
...
func (c *ObjectStoreController) migrateObjectStoreObject(objectstoreToMigrate *cephv1alpha1.ObjectStore) error {
	...
	err = c.context.RookClientset.RookV1alpha1().ObjectStores(objectstoreToMigrate.Namespace).Delete(objectstoreToMigrate.Name, nil)
	return err
}
