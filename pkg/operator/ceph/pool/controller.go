...
func (c *PoolController) onAdd(obj interface{}) {
	pool, migrationNeeded, err := c.getPoolObject(obj)
	if err != nil {
...
func (c *PoolController) onUpdate(oldObj, newObj interface{}) {
	oldPool, _, err := c.getPoolObject(oldObj)
	if err != nil {
		logger.Errorf("failed to get old pool object: %+v", err)
		return
	}
	pool, migrationNeeded, err := c.getPoolObject(newObj)
	if err != nil {
...
func (c *PoolController) getPoolObject(obj interface{}) (pool *cephv1alpha1.Pool, migrationNeeded bool, err error) {
	var ok bool
...
func (c *PoolController) migratePoolObject(poolToMigrate *cephv1alpha1.Pool) error {
	...
	err = c.context.RookClientset.RookV1alpha1().Pools(poolToMigrate.Namespace).Delete(poolToMigrate.Name, nil)
	return err
}
