...
func (c *ClusterController) onAdd(obj interface{}) {
	clusterObj, migrationNeeded, err := c.getClusterObject(obj)
	if err != nil {
	...
	if migrationNeeded {
		...
		if err != nil {
			logger.Errorf("failed to migrate cluster %s in namespace %s: %+v", clusterObj.Name, clusterObj.Namespace, err)
		}
...
func (c *ClusterController) onUpdate(oldObj, newObj interface{}) {
	oldClust, _, err := c.getClusterObject(oldObj)
	if err != nil {
		logger.Errorf("failed to get old cluster object: %+v", err)
		return
	}
	newClust, migrationNeeded, err := c.getClusterObject(newObj)
	if err != nil {
	...
	if migrationNeeded {
		if err = c.migrateClusterObject(newClust); err != nil {
			logger.Errorf("failed to migrate cluster %s in namespace %s: %+v", newClust.Name, newClust.Namespace, err)
		}
...
func (c *ClusterController) onDelete(obj interface{}) {
	clust, migrationNeeded, err := c.getClusterObject(obj)
	if err != nil {
		logger.Errorf("failed to get cluster object: %+v", err)
		return
	}
...
func (c *cluster) createInstance(rookImage string) error {
	...
	// Start the OSDs
	c.osds = osd.New(c.context, c.Namespace, rookImage, c.Spec.Storage, c.Spec.StoreConfig, c.Spec.DataDirHostPath, c.Spec.MetadataDevice,
